...
const int Nmax = 1000; //maximum amount of particles
...
int main()
{
	...
	for (int loop = 0;loop < 1;loop++) //multiple loops to gather data, set number after loop < ..., to set number of data folders
	{
		...
		while (nop < Nmax + 1) //timestep
		{
			...
			{
				if (ts % (save * 10) == 0) //print data at chosen timestep for vid
				{
			...
			for (int d = 0;d < nop;d++) //third loop to find defects
			{
				...
				if (ts % (4 * save) == 0)
					print.print_data_oop(ts, p[d].ID, p[d].xcpos, p[d].ycpos, p[d].theta, p[d].L, p[d].oop, p[d].ram); //save densities, in cluster 8 inputs are needed
