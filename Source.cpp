...
//Parameters
const double maxLoverD = 2.0; //gives relation between maxL and D, for rod-shaped particles: LD > 1
...
const double growthRate = 0.001 * (0.25 * (maxLoverD + diameter)) * tau * relaxTime; //number gives growth rate per timestep. experiments: 1.23 per hour, must be small compared to ki for relaxation
const double growthRateDev = 0.5 * growthRate; //sets deviation in growth rate
const double maxLengthDev = 0.01 * diameter * maxLoverD; //sets deviation in max length
const double orientNoise = 0.05; //sets value for noise in orientation of daughter cells to prevent growing in one line
...
const int Nmax = 64; //maximum amount of particles
...
void divide(Particle &pOld, Particle &pNew)
{
	...
	pNew.L = (oldL - pNew.D) / 2; //update new positions daughter cell, valid for LD > 1
...
int main()
{
	...
	int startloop = 0;
	for (int loop = startloop;loop < (startloop + 1);loop++) //multiple loops to gather data, set number after loop < (startloop + ...), to set number of data folders
	{
		cout << "Type of sim: loop nr = " << loop << ", nop = " << Nmax << ", mu = " << growthRate << ", muDev = " << growthRateDev << ", L/D = " << maxLoverD << ", Ldev = " << maxLengthDev << ", k_i = " << ki/tau << ", DF = sopram" << endl;
		int nop = 1; //number of particles is one at start
		int ts = 0; //timestep
		int save = 10; //determines how many times data is saved, for nop = 3000 time runs till 650000.
		...
		{
			if (ts % relaxTime == 0) //grow and relax system
			{
				for (int g = 0;g < nop;g++) //first loop to enable growth
				{
					p[g].neighbours.clear(); //clear neighbours vector
					p[g].grow();
					if (p[g].len > p[g].Lmax) //division, choose restlength or actual Length
					{
			...
			for (int m = 0;m < nop;m++) //second loop for movement of particles
			{
				...
				for (int nb = 0;nb < nop;nb++)
				{
					if (nb > m)
					{
						double distx = (p[m].xcpos - p[nb].xcpos) * (p[m].xcpos - p[nb].xcpos);
						double disty = (p[m].ycpos - p[nb].ycpos) * (p[m].ycpos - p[nb].ycpos);
						double range = 4 * p[m].Lmax * p[m].Lmax;
			...
			for (int d = 0;d < nop;d++) //third loop to find defects
			{
				...
				if (ts % (4 * save) == 0)
					print.print_data(ts, p[d].ID, p[d].xcpos, p[d].ycpos, p[d].theta, p[d].L, p[d].oop, p[d].ram); //save densities, in cluster 8 inputs are needed
		...
		int finish = time(0);
		cout << "nop is " << nop << ", elapsed time is "  << finish-start << endl;
	}
	system("pause");
