...
//Parameters
const double aspectRatio = 4; //gives relation between maxL and D, for rod-shaped particles: LD > 1
...
const double growthRate = 0.00004 * (0.25 * (aspectRatio + diameter)) * tau * relaxTime; //number gives growth rate per timestep. experiments: 1.23 per hour, must be small compared to ki for relaxation
const double growthRateDev = 0.1 * growthRate; //sets deviation in growth rate
const double maxLengthDev = 0.15 * diameter * aspectRatio; //sets deviation in max length
const double orientNoise = 0.1; //sets value for noise in orientation of daughter cells to prevent growing in one line

//Constants
const double maxLength = diameter * aspectRatio; //sets mean maximum length of particles
const double startLength = maxLength / 2; //starting length of first particle

const int Nmax = 10000; //maximum amount of particles
...
Particle::Particle(double xstart, double ystart, double angle, double length, double diameter, double growth)
{
	...
	oop = 0.0; ram = 0;
	colour = 160; pressure = 0.0; //used for visualisation
}
...
void forceOverlap(Particle &p1, Particle &p2, double Rx, double Ry, double dist, double rp1, double rp2)
{
	...
	p1.pressure += -ko * (dist - rlen);
	p2.pressure += -ko * (dist - rlen);
...
void distForce(Particle &p1, Particle &p2)
{
	...
	if (sqd < 3 * p1.D*p2.D) //Check if particles are neighbours, if so, append to neighbour vector
	{
...
void clearAll(vector<Particle> &p, int nop)
{
	for (int c = 0;c < nop;c++) //first loop to enable growth
	{
		p[c].clear();
	}
}
...
void moveAll(vector<Particle> &p, int nop)
{
	...
	for (int m = 0; m < nop; m++) //second loop for movement of particles
	{
		...
		{
			distx = (p[m].xcpos - p[nb].xcpos) * (p[m].xcpos - p[nb].xcpos); //determine if particles are in range for possible neighbour
...
void analyzeAll(vector<Particle> &p, int nop, Print &myPrint, int save, int ts)
{
	...
	{
		p[d].pressure = p[d].pressure / (2 * p[d].len + pi * p[d].D); //divide total overlap force by circumference
		p[d].colour = (int)(-p[d].pressure * 10000 + 160);
		if (p[d].colour < -60)
			p[d].colour = -60;
		...
		if (ts % save == 0)
			myPrint.print_data(ts, p[d].ID, p[d].xcpos, p[d].ycpos, p[d].theta, p[d].L, p[d].pressure, p[d].ram); //save densities, in cluster 8 inputs are needed
	}
}
...
int main()
{
	...
	int makefolder = 1; //leave as one to make folders
	int nop; //number of particles
	int ts; //timestep
	int save = 1000; //determines how many times data is saved, for nop = 3000 time runs till 650000. 300 gives nice set of data

	string folderName = "LD4_OvForce"; //name of data folder
	int startRound = 0;
	int noRounds = 5; //number of rounds
	for (int round = startRound; round < (startRound + noRounds); round++) //multiple rounds to gather data, set number after round < ..., to set number of data folders
	{
		...
		cout << "Mu = " << growthRate / relaxTime << ", muDev = " << growthRateDev << ", L/D = " << aspectRatio << ", Ldev = " << maxLengthDev << ", k_i = " << ki / tau << ", k_o = " << ko / tau << endl;
		cout << "DF = pressureByForce" << ", ON = " << orientNoise << endl;
